...
const JoinNetwork = () => {
    ...
    async function checkManager() {
        const checkValid = await fetch(`/api/user/checkValid/${manager}`);
        const checkValidResult = await checkValid.json();
        if (!checkValidResult.success) {
            alert('해당 매니저를 찾을 수 없습니다.');
            navigate("/main");
        }
    }
    ...
    async function handleJoin(e: React.FormEvent) {
        ...
        try {
            ...
            console.log('[JoinNetwork sendData]', JSON.stringify(sendData));

            const headers = { "Content-Type": "application/json" };
            const res = await fetch("/api/user/joinNetwork", {
                method: "post",
                headers: headers,
    ...
    return (
        <article className="join">
            <section className="network-title">
                ...
                <div>회원가입<p>Business Operator Network joining</p></div>
            </section>
